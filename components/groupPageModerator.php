<div class="flex flex-row items-center justify-between p-2 rounded-lg header-colorscheme">
    ...
    ?>
        <div class="flex flex-row gap-2">
            <form method="POST" action="<?= $context ?>/scripts/removeModerator.php" class="flex">
                <input type="hidden" name="groupId" value="<?= $groupId ?>">
                <input type="hidden" name="userNickname" value="<?= $userNickname ?>">
                <input type="hidden" name="groupRedirect" value="<?= $groupHandle ?>">
                <button type="submit" name="submitted" class="p-1 px-3 text-base font-bold text-center text-white transition-all duration-300 bg-red-400 rounded-full max-h-fit hover:bg-red-500 dark:bg-red-500 dark:hover:bg-red-600">Unmod</button>
            </form>
            <form method="POST" action="<?= $context ?>/scripts/kickFromGroup.php" class="flex">
                <input type="hidden" name="groupId" value="<?= $groupId ?>">
